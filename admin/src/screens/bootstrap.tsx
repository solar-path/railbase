...
// /_/bootstrap.
//
// v0.9 — wizard simplified back to TWO steps:
...
//
// Auth methods (LDAP, SAML, OAuth, etc.) and mailer (SMTP) configuration
// moved out of the first-run flow into Settings. The server-side gates
// in bootstrapCreateHandler (authGateError + mailerGateError) are also
// removed in v0.9 — admin creation no longer depends on these surfaces
// being touched.
//
// Each step owns its OWN useForm() — schemas and submit targets diverge
// enough that one mega-form would be more friction. Transient UI state
// (probe banners, save banners, detect result) stays on plain useState
// — it isn't form data.
...
// (authGateError + mailerGateError) were removed in the same change.
type Step = 0 | 1;
...
function DatabaseStep({ onContinue }: { onContinue: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/_admin/_setup/detect")
      ...
      .then((d: DetectResponse) => {
        if (cancelled) return;
        setDetect(d);
        if (d.current_mode === "embedded") {
          // Dev build on bundled Postgres — it's literally what we're
          // running on right now, so default to it (zero friction).
          form.reset({ driver: "embedded" });
        } else if (d.sockets.length > 0) {
          form.reset({
            driver: "local_socket",
        ...
        // Otherwise (current_mode "setup"/"external", no sockets) the
        // default external_dsn selection stays.
      })
      .catch((e: unknown) => {
        if (!cancelled) {
          setDetectErr(
  ...
  function bodyForBackend(values: DBStepValues) {
    ...
    if (values.driver === "embedded") {
      // No-op on the backend — /_setup/save-db short-circuits the
      // embedded driver and writes nothing. Sent for completeness so
      // the wire body always carries an explicit driver.
      return {
        driver: "embedded",
        socket_dir: "",
        username: "",
    ...
    // values.driver === "external_dsn" — the remaining union variant.
    return {
      driver: "external",
      socket_dir: "",
      username: "",
  ...
  function switchDriver(next: DBStepValues["driver"]) {
    ...
    } else if (next === "embedded") {
      form.reset({ driver: "embedded" });
    } else {
      // next === "external_dsn" — the remaining variant in the union.
  ...
  async function onSubmit(values: DBStepValues) {
    // Embedded postgres is already the running database whenever this
    // option is offered (current_mode === "embedded"). Nothing to
    // probe, save or restart — just advance to admin creation.
    if (values.driver === "embedded") {
      onContinue();
      return;
    }
  ...
  // Embedded is only a legitimate operator choice on dev builds — the
  // backend reports current_mode === "embedded" exactly when the binary
  // was built with `-tags embed_pg` and is running on bundled Postgres.
  const embeddedAvailable = detect?.current_mode === "embedded";
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              ...
              <FormItem>
                ...
                <FormControl>
                  <RadioGroup
                    value={field.value}
                    onValueChange={(v) => {
                      // Defensive: ignore options whose visual `disabled`
                      // / hidden cue could be bypassed by keyboard
                      // activation — local_socket when no sockets were
                      // detected, embedded when not a dev build.
                      if (v === "local_socket" && !hasSockets) return;
                      if (v === "embedded" && !embeddedAvailable) return;
          ...
          <div className="flex items-center gap-2 pt-2 border-t">
            {/* Embedded has no DSN to probe — buildSetupDSN rejects it.
                Hide the Probe button entirely for that driver. */}
            {driver !== "embedded" ? (
              <Button
                type="button"
                variant="outline"
