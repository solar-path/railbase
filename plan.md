...
| **v1.7.40 — Shareable shadcn-on-Preact UI kit: binary doubles as a UI registry + admin re-skinned (5 parallel work-streams, v1.x bonus)** | ✅ | 4 agents + 1 Claude inline closing the "Railbase ships a backend, give downstream apps a frontend kit too" gap. (a) **Port air's shadcn-on-Preact tree** (Claude): copied 50 components (button/card/input/table/dropdown-menu/select/sheet/sidebar/command/…) + 11 Radix-replacement primitives (slot/portal/popper/focus-scope/dismissable-layer/presence/…) + kit-base (cn/icons/theme/index) from `/Users/work/apps/air` into `admin/src/lib/ui/`. Skipped air's `Q*` composites (drizzle/oRPC-coupled, app-specific). `icons.tsx` is hand-rolled SVG — **zero lucide-preact dependency**. New peer deps: `class-variance-authority`, `clsx`, `tailwind-merge`, `@floating-ui/dom`, `react-hook-form`, `@hookform/resolvers`, `react-day-picker`, `embla-carousel`, `tw-animate-css`. (b) **Theme tokens** (Claude): rewrote `admin/src/styles.css` with full oklch palette (background/foreground/card/popover/primary/secondary/muted/accent/destructive/border/input/ring/chart-1..5/sidebar*) + light + dark mode + `@theme inline` mapping onto Tailwind's color system + `tw-animate-css` import. (c) **`admin/uikit.go` embed.FS** (Claude): `//go:embed all:src/lib/ui src/styles.css` exports `UIKit() fs.FS`. Lives in `admin/` package (not `internal/`) because `//go:embed` paths are directory-relative + cannot use `..`. Same package as `admin/embed.go` — binary now ships BOTH the compiled SPA AND the source TSX tree. Binary delta: ~250 KB; all 6 cross-compile targets still under 30 MB ceiling (largest 27.7 MB, headroom 2.3 MB). (d) **`internal/api/uiapi/`** (Claude): boot-time scanner (~360 LOC) + 10 HTTP handlers (~150 LOC). Scanner walks the embedded FS via `sync.Once`, classifies every `*.ui.tsx`'s `from '<pkg>'` imports into peers / primitives / kit-base / local siblings (both relative `./foo` and alias `@/lib/ui/foo` shapes recognised). Primitive peer deps (e.g. `@floating-ui/dom` via popper.tsx) folded into the kit's global peer set. Endpoints: `GET /api/_ui/{manifest,registry,components,components/{name}[/source],primitives[/{name}],cn.ts,styles.css,peers,init}` — all PUBLIC (published source = CDN-equivalent). `Cache-Control: public, max-age=300`. 11 tests in `registry_test.go` under `-race`. (e) **`railbase ui` CLI** (Claude): cobra subcommand `ui list|peers|init|add` in `pkg/railbase/cli/ui.go`. `ui init` scaffolds 14 files atomically (styles.css + cn/icons/theme/index + 11 primitives). `ui add NAME...` does BFS transitive resolution over `Local[]` — `ui add password` automatically pulls `input`; `ui add form` pulls `label`. Generic `keys[V any](map[string]V)` helper for the BFS work-queue. Pre-condition: `ui init` must have run (checks `cn.ts` existence; friendly error if missing). (f) **Source-of-truth cleanup** (Claude): deleted `admin/src/components/` (1 file — `password-input.tsx` was a worse-than-the-kit copy). Migrated 2 consumers (`screens/login.tsx` + `screens/bootstrap.tsx`) to `@/lib/ui/password.ui`; adapted API (`onValueChange`→`onInput`, `showGenerator`→`showGenerate`, `onGenerated`→`onGenerate`). Removed `lucide-preact` from package.json (zero consumers remained). Big docblock added to `admin/src/lib/ui/index.ts` stating the contract: this dir = source of truth, NEVER reach into `auth/api/fields/layout/screens` from here, app-specific composites go in `screens/` not `lib/ui/`. (g) **Admin re-skinned — 25/25 files** (4 parallel agents A/B/C/D + Claude inline): every admin screen + every layout file converted to consume the kit. Recipe applied consistently: `<button>` → `<Button variant="…" size="…">`, `<input>` → `<Input>`, `<table className="rb-table">` → `<Table>/<TableHeader>/<TableBody>/<TableRow>/<TableHead>/<TableCell>`, card-shaped divs → `<Card>/<CardHeader>/<CardContent>`, status pills → `<Badge variant="default|secondary|destructive|outline">`, raw color classes → theme tokens (`bg-neutral-*`→`bg-muted`, `text-red-*`→`text-destructive`, etc). Agent A (6 files: audit/logs/jobs/trash/backups/email-events, +75 LOC). Agent B (4 files: api_tokens/webhooks/notifications/mailer_templates, +49 LOC; built `ModalShell` over Card since kit has no `dialog.ui`). Agent C (6 files: dashboard/health/cache/realtime/schema/settings, +88 LOC). Agent D (6 files: records/record_editor/notifications-prefs/i18n/bootstrap/hooks; +44 LOC over 6 heavy files): records.tsx swapped tri-state header checkbox to kit `<Checkbox checked={"indeterminate"|bool}>` (no more imperative `el.indeterminate = …`); i18n.tsx locale picker moved from 240px vertical button-list → kit `<Select>` dropdown (the only "star widget" instance, ~80 LOC saved); bootstrap.tsx finished migration (driver radio → `<RadioGroup>`, "Create DB" checkbox → `<Checkbox>`); hooks.tsx `<TestPanel>` rebuilt on kit `<Collapsible>` (ARIA + chevron data-state handled by primitive). Monaco `<Editor>` lazy block untouched. Claude inline: 3 layout files (shell/pager/command_palette — kept hand-rolled keyboard nav in palette, only swapped color classes). **Pattern that emerged**: tables wrapped in `<Card><CardContent className="p-0 overflow-x-auto">` (no CardHeader because originals never had a title). **Open visual decisions flagged**: kit has no amber-token (3 different resolutions seen across batches — destructive when warn=problem; raw amber when warn=heads-up; raw amber-700 for audit error codes). Same gap for success-green. Kit has no `dialog.ui` (only alert-dialog + sheet/drawer) — `ModalShell` is a candidate primitive to upstream. Adding `--warning`/`--success` tokens + matching Badge variants + a proper `Dialog` is a candidate follow-up slice. Bundle delta: 79 KB gzip → 115 KB gzip (+36 KB for cva+clsx+tailwind-merge+imported kit components — pays off because all 25+ screens now share one Button/Card/Input/Table implementation instead of each hand-rolling its own). Combined: `go build ./...` + `go vet ./...` clean; `go test -race -count=1 ./internal/api/uiapi/...` + `./pkg/railbase/cli/...` green; `npm run build` green (177 modules, 422 KB JS / 115 KB gzip, 78 KB CSS / 14 KB gzip); cross-compile sweep all 6 binaries under 30 MB. Details: [progress.md#v1740](progress.md#v1740--shareable-shadcn-on-preact-ui-kit-50-components-served-by-the-binary--admin-re-skinned). |
| **v1.7.41 — react-hook-form across all admin screens + embedded-driver removed from production wizard** | ✅ | Closes the v1.7.40 deferred "adopt kit's react-hook-form integration in deep admin forms" follow-up. 9 admin screens migrated off `useSignal` + manual `onSubmit` onto kit `<Form>` + `useForm()` + `zodResolver` pattern (same recipe replicated for consistency). (a) **Migrated screens**: `bootstrap.tsx` (TWO useForm per step + `z.discriminatedUnion("driver", [local_socket, external_dsn])` + `.refine` для confirm-match + `setValue` fan-out on password-generate), `record_editor.tsx` (**dynamic zod** via `buildSchema(fields)` под `useMemo([editable])` + 422 server-error mapping на `form.setError(name, {type:"server", message})` из `e.body.details.errors`), `api_tokens.tsx` (create-modal + chained `useState` для display-once token banner), `webhooks.tsx` (`.refine` для http(s) URL validator), `notifications-prefs.tsx` (per-user settings form), `settings.tsx` (KV add-form + value-coercion w/ `form.setError("value")` на parse-failure), `i18n.tsx` (dynamic zod + flat-key cast `name={k as string}` обходит RHF auto-nested-paths для ключей с точками), `hooks.tsx` TestPanel (`z.string().refine` для JSON-object validation). (b) **Embedded-driver removed from production wizard** (operator pushback): wizard показывал disabled radio «Embedded postgres — not available in this build» в production-бинарнике, что путало. Третья ветка `dbStepSchema` (`z.literal("embedded")`) удалена целиком из `bootstrap.tsx`; `switchDriver()` / `bodyForBackend()` свёрнут до 2 веток; `defaultValues` сменился с `{driver: "embedded"}` на `{driver: "external_dsn", external_dsn: ""}`. Embedded postgres остаётся build-time decision через `-tags embed_pg` + `make build-embed` — отдельный dev-binary без wizard'a. (c) **`@hookform/resolvers` v5 gotcha**: zod `.default(false)` ломает `Resolver<...>` assignment (`.default` сплитит input vs output типы); workaround — НЕ использовать `.default()` в схемах, всё дефолтить через `useForm({ defaultValues })` + `form.reset()`. (d) **Bundle delta**: 25→237 модулей (+12 за RHF+resolvers+zod рёбра, paid one-time первой формой); 442→467.99 KB JS (+25.99 KB raw / 115→130.02 KB gzip). (e) **Docs**: `docs/12-admin-ui.md` Form strategy section расширен 6-pattern catalogue (static zod / dynamic zod / discriminated union / cross-field validation / server-error mapping / generator fan-out) + per-screen migration status table + v5-resolvers gotcha box. Details: [progress.md#v1741](progress.md#v1741--react-hook-form-across-all-admin-screens--embedded-driver-removed-from-production-wizard). |
| **v1.7.42 — Foreign-DB safety gate (probe-time scan + boot-time invariant + UI checkbox + `RAILBASE_FORCE_INIT` escape hatch)** | ✅ | Claude inline, 3-layer fix closing the only material UX risk in v1.7.39 setup wizard — "operator typo (`Database: billing_prod` instead of `Database: railbase`) silently points Railbase migrations + extensions at someone else's DB". Diagnosis: probe/save endpoints have NO destructive ops (no DROP/TRUNCATE), so data-loss isn't the risk — schema pollution is. (a) **Probe-time scan** (`setup_db.go`): `setupProbeResponse` gains `public_table_count int` + `is_existing_railbase bool`; one extra `pg_tables` round-trip after `SELECT version()`. Best-effort — managed-DB catalog visibility failures leave fields at zero, wizard falls back to neutral path. (b) **Boot-time invariant** (`internal/db/migrate/migrate.go`): new `ErrForeignDatabase` sentinel + `Runner.AllowForeignDatabase bool`. `Apply()` calls `checkForeignDatabase(ctx)` BEFORE `bootstrap()`. Catches the manual-`.dsn`-edit route that bypasses the wizard. Same `pg_tables` query; failure non-fatal (logs + proceeds for managed-DB compat). (c) **UI checkbox gate** (`bootstrap.tsx`): `ForeignDbWarning` amber block when `count>0 && !is_existing_railbase` — shows table count + explains pollution risk + locks Save behind acknowledgement checkbox. `ExistingRailbaseNotice` green block when marker found. `proceedAnyway` resets on every probe so changing DB name + re-probing doesn't leak prior consent. (d) **Escape hatch** (`app.go`): `RAILBASE_FORCE_INIT=1` → `AllowForeignDatabase=true`. Env var (not CLI flag) on purpose — operator should feel the friction of "doing something unusual". (e) **Tests**: 4 new in `internal/db/migrate/foreign_db_test.go` (FreshDB / ExistingRailbase / ForeignTables / AllowOverride; embed_pg own TestMain with `os.Exit(runTests(m))` defers-leak fix) + 3 new in `setup_db_embed_test.go` (Scan_ExistingRailbase / Scan_FreshDB / Scan_ForeignDB). All green under `-race`. (f) **Docs**: `docs/12-admin-ui.md` First-run experience section rewritten to document the three safety states (green/amber/neutral) operator sees in the Database step. Schema-isolation approach (`CREATE SCHEMA railbase`) explicitly rejected — doesn't solve CREATE EXTENSION pollution (DB-scope not schema-scope), over-engineering for an early product. Separate `_railbase_instance` fingerprint table NOT added — `_migrations` already serves as marker; instance_id UUID is a v1.x candidate when backup/restore distinguishes Railbase installs against the same DB. `make verify-release` green: largest binary 27.79 MB Windows amd64, headroom 2.21 MB. Details: [progress.md#v1742](progress.md#v1742--foreign-db-safety-gate-probe-time-scan--boot-time-invariant--ui-checkbox--railbase_force_init-escape-hatch). |
| **v1.7.43 — Mandatory email on admin creation: wizard mailer step + welcome + broadcast notice + retry sweeper + `_admins` namespace clarification** | ✅ | Closes the operator-raised "при создании учётной записи отправка email строго обязательно" gap. Pre-v1.7.43 neither bootstrap nor `railbase admin create` sent any email — operator's MailHog inbox showing 0 messages was correct behaviour but felt wrong; Auth0/Cognito/Supabase/Keycloak all email on admin-create by default. (a) **Wizard expansion 2→3 steps**: Database → **Mailer** → Admin. New `MailerStep` component (~530 LOC in `admin/src/screens/bootstrap.tsx`) — SMTP/console driver picker + per-driver form + "Send test email" button + "Save and continue" + amber "Skip — I'll configure later" confirm modal w/ mandatory reason. Pre-fills from masked snapshot on return-visits (password never echoed back). (b) **Backend `internal/api/adminapi/setup_mailer.go`** — 4 PUBLIC endpoints (`mailer-status` / `mailer-probe` / `mailer-save` / `mailer-skip`). probe builds ephemeral `mailer.Driver` + attempts test send to `probe_to` (never writes settings); save persists every key via `Manager.Set` calls (each fires `settings.changed` event) + stamps `mailer.configured_at` + **clears** `mailer.setup_skipped_at` (intent-reversal); skip stamps `setup_skipped_at`+`reason` w/ ≤500 chars + non-empty validation. (c) **Bootstrap gate** — `bootstrapCreateHandler` calls `mailerGateError(ctx, d)` BEFORE body decode. Returns NEW `CodePreconditionFailed` → HTTP 412 if neither `mailer.configured_at` nor `mailer.setup_skipped_at` is set. Gate passes → enqueue 2 emails: `admin_welcome.md` to new admin (login URL + 2FA reminder) + `admin_created_notice.md` broadcast to every OTHER admin (compromise detection). Both via `send_email_async` (v1.7.30) w/ `MaxAttempts: 24` so welcomes survive ~24h SMTP downtime via standard exp-backoff alone. (d) **CLI `admin create`** — same emails + new `--no-email` flag (per-invocation suppression for scripts). CLI bypasses the 412 gate on purpose (operator surface) but honours `mailer.setup_skipped_at` flag. `pkg/railbase/cli/admin_emails.go` is the CLI-local twin — replicated rather than imported because CLI shouldn't depend on adminapi (chi etc.). (e) **Retry sweeper** `internal/jobs/welcome_email_retry.go` — new `retry_failed_welcome_emails` job kind registered alongside `RegisterMailerBuiltins` w/ default schedule `*/30 * * * *`. ONE UPDATE flips `status='failed'`→`pending` for welcome-family rows that meet all of (kind=send_email_async / template IN (admin_welcome, admin_created_notice) / NOT ILIKE '%permanent failure%' / completed_at < now()-15min / created_at > now()-7d). Welcome-only — password_reset / signup_verification stay untouched (stale content + expired links). (f) **Templates** — `admin_welcome.md` + `admin_created_notice.md` in `internal/mailer/builtin/` (auto-discovered via existing `//go:embed builtin/*.md`). Only `{{ key }}` interpolation (template engine doesn't support `{{#if}}`). (g) **Tests**: 8 pure-unit (`setup_mailer_test.go`) + 6 embed_pg (`setup_mailer_embed_test.go` — skip/save persistence + 3 bootstrap-gate state matrix) + 7 embed_pg (`welcome_email_retry_test.go` w/ own TestMain — happy / broadcast / leaves-password-reset / ignores-permanent / ignores-fresh / ignores-stale / empty-table). All green under `-race`. (h) **Docs**: `docs/04-identity.md` — System vs Domain accounts quick reference + corrected stale `_system_admins` → `_admins` mentions; `docs/03-data-layer.md` — reserved `_*` namespace callout listing all 27 system tables; `docs/14-observability.md` — new "Mandatory email on admin creation (v1.7.43)" subsection w/ templates table + delivery durability + endpoint reference. Bundle delta: 467.99 → 480.13 KB raw / 130.02 → 132.52 KB gzip (+2.5 KB). Details: [progress.md#v1743](progress.md#v1743--mandatory-email-on-admin-creation-wizard-mailer-step--welcome--broadcast-notice--retry-sweeper--_admins--app-users-namespace-clarification). |
| **v1.7.44 — Default port migration `:8090` → `:8095`** | ✅ | Pure default-value swap. PocketBase shares `:8090`; operators migrating routinely keep both running. `:8095` is IANA-unassigned, no default daemon on Linux/Windows/macOS, no collision with the usual dev/prod tooling (Postgres :5432 / Redis :6379 / nginx :80/8080 / Prometheus :9090 / Cassandra :7000 / Vite :5173 / etc.). Touched: `internal/config/config.go` (`Default().HTTPAddr` + rationale comment), `internal/openapi/openapi.go` (default `servers[0].url`), `internal/api/adminapi/{bootstrap,setup_db}.go`, `internal/security/antibot.go`, `pkg/railbase/app.go::preflightBindCheck` (fully rewritten — drops misleading "PocketBase blame", adds Windows `netstat -ano | findstr` alongside Linux/macOS `lsof`), `pkg/railbase/cli/{serve,generate,admin_emails}.go`, `admin/playwright.config.ts`, `scripts/smoke-5min.sh`, `docker-compose.yml`, 4 docs files. Override path intact: `RAILBASE_HTTP_ADDR=:8090 ./railbase serve` returns the old default in one line. All 6 cross-compile binaries pass under 30 MB ceiling (no measurable size impact — constant-string substitution). Details: [progress.md#v1744](progress.md#v1744--default-port-migration-8090--8095). |
| **v1.7.45 — Enterprise SSO core: SAML 2.0 SP + LDAP + SCIM 2.0 (3 parallel tracks moved INTO core, NOT plugins)** | ✅ | Operator pushback закрыл вопрос "почему SAML/LDAP/SCIM — плагины, а OAuth — ядро?" — для типового enterprise-проекта эти три обязательны как HTTP сервер, не как опция. Решение: всё переместилось в `internal/auth/{saml,ldap,scim}` рядом с oauth/, builds в каждый бинарник. (a) **SAML 2.0 SP** — `internal/auth/saml/saml.go` (~1300 LOC): IdP-initiated + SP-initiated AuthnRequest + ACS endpoint + SLO (Single LogOut) + signed AuthnRequests (per IdP requirement) + SP metadata generation + InResponseTo binding for replay protection + group → role mapping via `cfg.GroupAttribute`. `internal/api/auth/saml_flow.go` wires HTTP handlers. Settings keys `auth.saml.{enabled,entity_id,acs_url,idp_sso_url,idp_slo_url,idp_certificate,sp_certificate,sp_private_key,group_attribute,group_role_map,sign_authn_requests,nameid_format}`. `pkg/railbase/saml_wiring.go` reads settings + injects into `auth.Deps.SAML`. Goxmldsig native — no encrypted assertions (xmlenc deferred). 6 unit + 1 Keycloak e2e (v1.7.47). (b) **LDAP** — `internal/auth/ldap/ldap.go` (341 LOC): service-account bind → user search → optional secondary bind for password validation → group resolution via `User.DN` lookup. TLS modes: off/STARTTLS/LDAPS. `internal/api/auth/ldap_flow.go` POST `/auth-with-ldap` handler. Settings `auth.ldap.{enabled,addr,base_dn,bind_dn,bind_password,user_filter,group_filter,tls_mode,attribute_email,attribute_displayname}`. Tested against OpenLDAP + Active Directory schemas. NO connection pooling (deliberate — first connection per-request stays under 50ms vs pool maintenance complexity for `enterprise SSO` low-traffic regime). (c) **SCIM 2.0** — `internal/auth/scim/scim.go` types + filter parser + `internal/api/scim/{users,groups,discovery,role_mapping,mount}.go` (~1500 LOC) full RFC 7643/7644 implementation: `/scim/v2/{Users,Groups,Schemas,ResourceTypes,ServiceProviderConfig}` w/ POST/GET/PUT/PATCH/DELETE. SCIM filter parser supports `eq/ne/co/sw/ew/gt/ge/lt/le/pr/and/or/()/[]`. Migration 0026 adds `_scim_tokens` (HMAC-SHA-256 hashed, `rbsm_`-prefix) + `_scim_group_role_map` (group_id → role_id reconciliation). `internal/auth/scim/tokens.go` provides Create/Rotate/Revoke/List. **Group → role reconciliation** (`role_mapping.go` 80+ LOC) fires on every Group create/PATCH/PUT/DELETE via `reconcileGroupGrants`: walks `_scim_group_role_map` entries → `rbacStore.Assign/Unassign` w/ a "user still has role via OTHER mapped group" residual check so removing user from one group doesn't drop a shared role. Discovery `.ServiceProviderConfig` explicitly says `sort=false, etag=false` (most IdPs ignore; low priority). 7 unit + 6 e2e (v1.7.47). (d) **Admin setup wizard auth step** — `internal/api/adminapi/setup_auth.go` GET/PUT `/api/_admin/_setup/auth`: 9 toggles (password / magic_link / otp / totp / webauthn / oauth.{google,github,apple,oidc} / saml / ldap / scim) + per-provider config blob. PUT writes via `settings.Manager.Set` (each fires `settings.changed`). `admin/src/screens/bootstrap.tsx` step 3 of 4-step wizard. (e) **Mailer SMTP presets** — `internal/api/adminapi/setup_mailer.go::presets` returns `{name, host, port, encryption, hints}` array for Google (smtp.gmail.com:587, STARTTLS, App Password hint) / MailHog (localhost:1025, none, dev) / Custom (operator fills). Wizard step 2 dropdown auto-fills fields. (f) **.env support** — `internal/config/dotenv.go`: `Load()` reads `.env` after defaults + before `RAILBASE_*` env vars; KEY=VALUE format only, no shell expansion. Dev convenience. (g) **CLI** — `railbase scim token create/list/revoke/rotate` (`pkg/railbase/cli/scim_token.go`). LDAP + SAML config via `railbase config set auth.{ldap,saml}.<key>=<value>` (no dedicated CLI — settings.Manager Set surface is universal). Bundle: 480 → 552 KB raw / 132 → 149 KB gzip (admin wizard step + setup_auth UI). Binary: +~2 MB across all 6 targets (SAML + SCIM XML/JSON parsers + LDAP client = bulk). Details: [progress.md#v1745](progress.md#v1745--enterprise-sso-core). |
| **v1.7.46 — Admin redesign 3 parallel tracks: System tables + Backend metrics + Color tokens ESLint enforcement + admin recovery flows + HMR dev mode** | ✅ | 3 agents + Claude inline wiring/polish. Closes §3.11 admin UI gaps (system tables / metrics / mailer template editor / password reset flows) AND introduces strict color token discipline. (a) **System tables track** — `internal/api/adminapi/system_tables.go` (~400 LOC) read-only REST views: `GET /api/_admin/system/{admins,sessions,admin_sessions}` w/ pagination + filters. Screens `admin/src/screens/system_{admins,sessions,admin_sessions}.tsx`: live tables w/ revoke session affordance (hits existing `_admin_sessions` revoke endpoint). NOT writable surface — admin lifecycle stays on `railbase admin create/delete` CLI. (b) **Backend metrics track** — `internal/metrics/` package + `internal/api/adminapi/metrics.go`: in-process atomic counters for goroutine count / heap MB / GC pause / requests/min / DB pool stats (active/idle/waiting/acquired-since-boot) / job runs/min / hooks dispatch latency. `GET /api/_admin/metrics` snapshot + `admin/src/screens/dashboard.tsx` polling 5s. (c) **Color tokens enforcement** — `admin/eslint.config.js` + custom `admin/eslint-rules/no-raw-colors.js`: rule blocks any `bg-red-*`/`text-amber-*`/`border-blue-*` etc. outside `admin/src/lib/ui/` (theme tokens only). Forces every screen through `bg-destructive`/`text-muted-foreground`/`border-input` etc. `npm run lint:eslint` clean (0 warnings/errors). (d) **Admin recovery flows** — `internal/api/adminapi/forgot_password.go` POST `/api/_admin/forgot-password` issues `_record_tokens` w/ purpose `admin_password_reset`; POST `/api/_admin/reset-password` consumes token + bcrypt updates `_admins.password_hash` + revokes all admin sessions. `admin/src/screens/{forgot_password,reset_password}.tsx` UI. Mailer builtin `admin_password_reset.md`. (e) **Mailer template editor** — `admin/src/screens/mailer.tsx` list-view with override-status badges per builtin template + per-template raw markdown ↔ rendered HTML preview. Editing-on-disk surface stays — operator pattern is "edit `.md` + hot-reload via existing mailer loader" (intentionally no save-via-UI to keep template source in git, not DB). (f) **Sidebar shadcn alignment** — `admin/src/lib/ui/sidebar.ui.tsx` rewritten to match shadcn 1:1: `SIDEBAR_WIDTH/_MOBILE/_ICON` constants, CSS vars on `SidebarProvider` wrapper, cookie `sidebar_state`. Closes operator pushback "большие отступы". (g) **HMR dev mode** — `make dev` / `make dev-embed` Makefile targets: auto-detect `postgres://$(whoami)@/railbase?host=/tmp&sslmode=disable` brew socket OR fall back to embedded; auto-`migrate up` before backend boot; `RAILBASE_HTTP_ADDR=:8095` env (no `--addr` flag — serve cmd doesn't accept); `pkill -f 'bin/railbase serve' && lsof -ti 5173 \| xargs kill -9` cleanup; `admin && bun run dev --strictPort` w/ Vite proxy for `/api`, `/readyz`, `/healthz`, `/scim`. One-command dev — closes "перезапускать билд после каждой правки". (h) **Tests**: 7 unit `system_tables_test.go` + 6 unit `metrics_test.go` + 8 unit `forgot_password_test.go` + 6 unit `setup_auth_saml_advanced_test.go` (SLO URL / group mapping / SP cert / SP key not echoed / preserve-on-empty / sign_authn_requests validation) + 35-check `scripts/test-wizard-keycloak.sh` integration harness against live Keycloak (LDAP / SAML / SCIM all roundtripped against `/Users/work/apps/keyclock/` docker stack). Details: [progress.md#v1746](progress.md#v1746--admin-redesign-3-tracks). |
| **v1.7.47 — "Что НЕ покрыто" closure: Keycloak browser e2e for SAML + OIDC + SCIM group → role reconciliation tests (3 phases)** | ✅ | Closed three gaps flagged in the v1.7.46 wizard audit. (a) **Phase A: SCIM role-mapping reconciliation** — `internal/api/scim/role_mapping.go` (~80 LOC): `reconcileGroupGrants(ctx, pool, rbacStore, collection, groupID, userID, added)` + `mappedRolesForGroup` + `userHasMappedRoleViaOtherGroup` residual check. Wired in `groups.go` at ALL 4 CRUD points (create / PATCH / PUT / DELETE) — `groups.go::createGroup` reconciles each new member; `replaceGroup` (PUT) diffs old↔new + reconciles both adds and removes; `patchGroup` (PATCH) collects `memberDelta` during tx + reconciles after commit; `deleteGroup` snapshots members + mapped roles before DELETE + revokes after. 6 e2e in `role_mapping_e2e_test.go`: PATCH-add-grants / PATCH-remove-revokes / role-survives-removal-via-other-group / PUT-full-replace / DELETE-revokes-all / nil-RBAC-no-op. (b) **Phase B: SAML browser flow** — `internal/api/auth/saml_browser_flow_test.go` under `embed_pg && keycloak_integration` build tag. Full 8-step browser dance against live Keycloak (`localhost:8081` realm `grc-test` user `alice.anderson`): register SAML client via admin REST → GET `/auth-with-saml/start` → 302 to Keycloak `/protocol/saml` → scrape login form HTML → POST creds → 302 to ACS w/ SAMLResponse → handler parses + validates + creates JIT user + sets cookie → `/me` 200 + `_external_auths` linkage row. Helpers: `keycloakAdminToken/registerKeycloakSAMLClient/deleteKeycloakClient/extractFormAction/extractHiddenInput/mustURL/head/bootRailbasePool/mountAuthForSAMLTest`. Settings wiring: `deps.Settings.Set(ctx, "auth.saml.enabled", true)` + `authmw.New(deps.Sessions, deps.Log)` in test mux. (c) **Phase C: OAuth/OIDC Keycloak** — `internal/api/auth/oauth_oidc_keycloak_test.go` same build tag. 8-step OIDC dance via `oauth.Generic` provider w/ Keycloak discovery URLs (`auth/token/userinfo`). Two debugging finds: (i) `deps.PublicBaseURL = srv.URL` REQUIRED — `oauthRedirectURI()` defaults to `http://localhost:8080` else Keycloak responds "Invalid parameter: redirect_uri"; (ii) step 4 callback returns 200 + JSON `{token, record}` (no return_url) — NOT 302 like SAML. Assertion relaxed to accept either. Verifies `_external_auths` row + JIT user + `/me` cookie. (d) **`make verify-release` post-Phase A/B/C**: green. All 6 cross-compile binaries under 30 MB ceiling — largest 29.42 MB Windows amd64, smallest 26.88 MB linux arm64, **headroom 0.58 MB** (down from 3.57 MB at v1.7.29 — Enterprise SSO core consumed ~3 MB across all targets). ESLint 0 warnings. Admin bundle 552.75 KB main + 338.23 KB lazy chart (gzip 149.66 + 102.33 KB). Details: [progress.md#v1747](progress.md#v1747--что-не-покрыто-closure-keycloak-e2e--scim-role-mapping). |
| **v1.7.48 — Block A §3.15 burndown: SCIM soft_delete + Audit XLSX export + SCIM sort/etag RFC 7644 (3 parallel sub-slices)** | ✅ | 3 параллельных агента закрывают 3 из 10 пунктов §3.15 Block A. (a) **SCIM `auth.scim.soft_delete` toggle** — `internal/api/scim/users.go::deleteUser` теперь читает `auth.scim.soft_delete` boolean из Deps.Settings (default `false` для backward-compat); при `true` + collection с `.SoftDelete()` flag — UPDATE `deleted=now()` вместо DELETE; reconcileGroupGrants drops всё равно прокатываются (user перестаёт быть активным). Graceful fallback на жёсткое удаление если collection не SoftDelete. 4 e2e под `embed_pg` (hard-when-off / soft-when-both / hard-when-collection-not-softdelete / role-grants-revoked-on-soft). (b) **Audit log XLSX export** — новый `internal/api/adminapi/audit_export.go` (~349 LOC) handler `GET /api/_admin/audit/export.xlsx` стримит `_audit_log` через `excelize.StreamWriter` (reuse `internal/export`); ту же query-grammar что `auditListHandler` (event/outcome/user_id/since/until/error_code); hard cap 100k строк w/ `X-Truncated: true`+`X-Row-Cap` headers; filename `audit-YYYY-MM-DD.xlsx`; audit-of-audit `audit.exported` event пишется ПОСЛЕ успешного flush. Admin UI `audit.tsx` получил кнопку "Export XLSX" (kit `Button variant="outline" size="sm"` + `Download` icon). 4 e2e + admin build clean. Bundle delta +0.95 KB raw / +0.31 KB gzip. (c) **SCIM sort + etag RFC 7644** — `internal/api/scim/etag.go` (новый) ship `etagFor(updated time.Time) string` weak ETag `W/"<epoch_ms>"`. `listUsers`/`listGroups` parsят `?sortBy=` (whitelist: `userName`/`emails.value`/`id`/`meta.created`/`meta.lastModified`) + `?sortOrder=ascending|descending`; неизвестный sortBy → 400 InvalidValue. `getUser`/`getGroup` всегда добавляют `ETag` header; `If-None-Match` совпадение → 304 Not Modified; PUT/PATCH с устаревшим `If-Match` → 412 PreconditionFailed (SCIM error envelope); `If-Match: *` wildcard → всегда match. `ServiceProviderConfig.sort.supported`+`etag.supported` обновлены `false`→`true`. 8 e2e (sort-asc/sort-desc/unknown-field-400/order-without-by-noop/etag-header/if-none-match-304/if-match-mismatch-412/if-match-wildcard-ok). **Бонусом fixed 3 stale tests caused by v1.7.45/v1.7.47 upstream**: `TestSetupAuthEmbed_StatusDefaults` (теперь expects `plugin_gated` empty — Enterprise SSO в ядре) + `TestBootstrapMailerGate_AcceptsWhenConfigured`/`AcceptsWhenSkipped` (добавлен `auth.configured_at` / `auth.setup_skipped_at` чтобы пройти новый orthogonal `authGateError` gate в `bootstrap.go`). `make verify-release` green: все 6 cross-compile binaries под 30 MB ceiling (darwin_amd64 29.37 / linux_amd64 28.70 / windows_amd64 29.46 / etc., headroom 0.54 MB, минимальный +0.03-0.04 MB delta vs v1.7.47). |

**Отгружено**: v0 (~17.9k LOC Go + ~1.5k LOC TS) + v1 milestones (~21k+ LOC Go) + **v1.7.40 added a Preact+shadcn UI kit raised from `/Users/work/apps/air`** that ships embedded in the binary AND is reachable over HTTP at `/api/_ui/*` / via `railbase ui` CLI — Railbase is now ALSO a UI registry, not just a backend. **v1.7.41** migrated 9 admin screens to `react-hook-form` + `zodResolver` (closes UI kit deferred follow-up; embedded-driver radio removed from production wizard per operator pushback). **v1.7.42** added a 3-layer foreign-DB safety gate (probe-time scan + boot-time invariant in `internal/db/migrate` + UI checkbox + `RAILBASE_FORCE_INIT=1` escape hatch) closing the only material UX risk in the v1.7.39 setup wizard. **v1.7.43** made email-on-admin-creation mandatory — wizard expanded to 3 steps (Database → Mailer → Admin), 412 PreconditionFailed gate on bootstrap-handler, dual welcome+broadcast notice emails, retry sweeper (`retry_failed_welcome_emails` every 30 min), CLI `--no-email` escape hatch, `docs/04-identity.md` rewritten to clarify `_admins` (system) vs app-collections (`users`/`customers`/…) namespace separation. **v1.7.44** swapped default HTTP port `:8090` → `:8095` (PocketBase-coexistence). **v1.7.45 переместил Enterprise SSO в ядро**: SAML 2.0 SP (~1300 LOC, IdP+SP initiated, ACS, SLO, signed AuthnRequests, InResponseTo replay protection, group→role mapping) + LDAP bind/search (341 LOC, TLS/STARTTLS/LDAPS) + SCIM 2.0 (~1500 LOC, RFC 7643/7644 Users+Groups full CRUD + filter parser + token mgmt + group→role reconciliation) — раньше предполагались плагинами, теперь in-tree во всех 6 cross-compile binaries (бинарник +~2 MB / target, headroom 30 MB ceiling → 0.58 MB). Wizard расширен до 4 шагов (Database → Mailer → Auth → Admin); mailer step получил SMTP presets (Google / MailHog / Custom); `.env` файлы теперь подхватываются. **v1.7.46** закрыл §3.11 admin-UI gaps тремя параллельными агентами: System tables (read-only views на `_admins`/`_sessions`/`_admin_sessions`) + Backend metrics (`internal/metrics/` + dashboard polling 5s) + Color tokens ESLint enforcement (custom rule блокирует raw цветовые классы вне `lib/ui/`) + admin recovery flows (forgot/reset password) + mailer template editor + sidebar shadcn alignment + HMR dev mode (`make dev`). **v1.7.47** закрыл три gap'а "Что НЕ покрыто": SCIM group→role reconciliation (`role_mapping.go` + 6 e2e) + SAML browser flow против live Keycloak (8 шагов под `embed_pg && keycloak_integration` build tag) + OAuth/OIDC Keycloak (8 шагов). **49 пакетов с тестами** (+1 `internal/api/uiapi`, +1 `internal/auth/saml`, +1 `internal/auth/ldap`, +1 `internal/auth/scim`, +1 `internal/api/scim`, +1 `internal/metrics`), все green под `-race`. v0 released; v1.0–v1.7.47 shipped through autonomous loop slices — see milestone table above and [progress.md](progress.md) for per-slice deltas. **Post v1.7.47 honest completion of plan.md v1 scope: ~99.9%** (~155 of ~155 currently-scoped v1 line items shipped + Enterprise SSO core promoted INTO v1 from former v1.1+ plugin track — UI kit + form-strategy + foreign-DB safety + mandatory-email + Enterprise SSO are all v1.x bonus polish, parallel track to SHIP). v1.7.38-39 closed §3.2.10 Auth origins, §3.4.5 onRequest, §3.4.10 Go-side typed hooks, §3.4.11 hook test panel + the first-run DB setup wizard (new architecture, "universal backend, не зашивай db-name+user в boot"). §3.5.9 PB-SDK realtime compat closes 8/9 sub-items. Remaining `📋` markers are explicit v1.x-bonus / v1.1+ / v1.2+ / dependency-blocked items the original roadmap put out of v1 scope. Recently closed in v1.7.31-v1.7.35 bonus burndown (**~26 sub-slices in single autonomous session**): §3.1.4 `_email_events` durable bounce/delivery store, §3.1.6 Mailer hooks dispatcher, §3.4.3 goja stack-cap (partial — goja has no true mem limit), §3.4.5 onAuth event topics (partial — onMailer ✅, onAuth ✅; onRequest still 📋), §3.5.3 WebSocket transport alongside SSE, §3.6.13 orphan_reaper, §3.7.5.2 scheduled_backup, §3.7.5.3 audit_seal Ed25519 chain (pulls part of §4.10/v1.1 into v1.x), §3.9.1 quiet-hours + digests + admin-side prefs editor, §3.9.3 ICU plurals + `.Translatable()` end-to-end, §3.9.4 Cache wiring slices 1+2 (4 caches: filter.ast / rbac.resolver / i18n.bundles / settings), §3.9.5 Anti-bot middleware (honeypot+UA), §3.12.1 `railbase test` CLI, §3.12.2 YAML fixtures, §3.12.5 gofakeit mock data, §3.12.7 `railbase coverage` combined Go+JS HTML report, §3.12.8 MockHookRuntime, plus `jobs.ErrPermanent` sentinel + mailer→jobs cross-package permanent-error promotion + RBAC bus-driven invalidation + `webhook.delivered` topic + TestMain os.Exit-defers hardening + pre-existing oauth flaky-base64-padding test fix.

§3.8 Domain types 8/9 groups fully closed (Hierarchies 4/7); **field-renderer admin UI track functionally complete at 25/~25 types** (slices 1-5 v1.7.25b through v1.7.29b). §3.9 Notifications + webhooks + i18n + cache + soft-delete + CSRF + security headers — functionally complete. §3.10 Document generation 8/8 ✅. §3.13 PB-compat track 9/9 ✅ — feature parity complete. §3.5.x SSE resume tokens shipped (WS deferred to v1.x). **§3.11 Admin UI track at 22/25 listed screens shipped** (~88%): Audit list+filters v0.8/v1.7.14 / Logs v1.7.6 / Jobs v1.7.9a / API tokens v1.7.9b / Shared Pager v1.7.9c / Backups + Notifications v1.7.11 / Trash v1.7.14 / Mailer templates v1.7.15 / Realtime monitor v1.7.16 / Webhooks management v1.7.17 / Command palette ⌘K v1.7.18 / Hooks editor v1.7.20b / Translations editor v1.7.22b / Health dashboard v1.7.23c / Cache inspector v1.7.24b / Field renderers v1.7.25b-v1.7.29b + **v1.7.46a System admin tables / v1.7.46b Backend metrics dashboard / v1.7.46d Admin recovery flows / v1.7.45+39+43 4-step Setup wizard / v1.7.46c Color tokens ESLint enforcement**. **3 remaining 📋 are all blocked-by-design**: Documents browser (blocked on §3.6 Documents track — out of v1) / Hierarchical tree-DAG viz (blocked on §3.8 Hierarchies tail — deferred v1.6.x) / Realtime collab indicators (v1.1+). §3.12 Testing infrastructure ✅ Go-side (v1.7.20a `testapp` JSON + v1.7.30b YAML fixtures + v1.7.28c `railbase test` CLI) — JS-side `mockApp().fireHook()` deferred. §3.14 verification gate — **✅ test-debt + ⏭ SHIP-gate empty post-v1.7.24; CI per-PR binary-size enforcement added v1.7.28a; `make verify-release` end-to-end smoke validated v1.7.29a**. **v1 SHIP fully unblocked + operator release artifacts + per-PR size gate + pre-tag one-command gate all green.** Honest completion of v1 plan.md scope: **~87% (~130 of ~150 line items shipped)**; remaining ~20 split between dependency-blocked / explicit v1.1+ / agent-friendly v1.x-bonus. Continuing burndown via parallel slices.
...
### 3.2 ✅ Auth providers + flows (v1.1) + Enterprise SSO (v1.7.45)

Phase split: **Phase A** (record-token flows) — ✅; **Phase B** (OAuth/TOTP/WebAuthn/MFA/RBAC) — sub-milestones v1.1.1–v1.1.4 ✅; **Phase C** (Enterprise SSO — SAML/LDAP/SCIM) — v1.7.45 ✅ (промотировано из v1.1+ plugin track в ядро).
...
| 3.2.11 | Devices, invites, impersonation | B | 📋 v1.1.x (deferred — admin UI: devices list / impersonation start/stop; CLI candidate when needed) |
| **3.2.13** | **SAML 2.0 SP** — `internal/auth/saml/saml.go` ~1300 LOC, IdP-initiated + SP-initiated AuthnRequest + ACS + SLO + signed AuthnRequests + SP metadata + InResponseTo replay protection + group→role mapping via `cfg.GroupAttribute`. Settings keys `auth.saml.*` w/ `pkg/railbase/saml_wiring.go`. 6 unit + 1 Keycloak e2e (`saml_browser_flow_test.go` 8 шагов под `embed_pg && keycloak_integration`). | C | ✅ (v1.7.45 + v1.7.47) |
| **3.2.14** | **LDAP / Active Directory** — `internal/auth/ldap/ldap.go` 341 LOC, service-account bind + user search + group resolution + TLS off/STARTTLS/LDAPS. `POST /auth-with-ldap` handler. Tested against OpenLDAP + Active Directory schemas. NO connection pooling (deliberate v1.x deferral для low-traffic enterprise regime). | C | ✅ (v1.7.45) |
| **3.2.15** | **SCIM 2.0 inbound provisioning** — `internal/auth/scim/` types + filter parser + `internal/api/scim/{users,groups,discovery,role_mapping,mount}.go` ~1500 LOC, RFC 7643/7644 full implementation. POST/GET/PUT/PATCH/DELETE `/scim/v2/{Users,Groups}` + `.Schemas` / `.ResourceTypes` / `.ServiceProviderConfig`. SCIM filter `eq/ne/co/sw/ew/gt/ge/lt/le/pr/and/or/()/[]`. Migration 0026 adds `_scim_tokens` (HMAC-SHA-256 hashed, `rbsm_`-prefix) + `_scim_group_role_map`. Group→role reconciliation w/ "user has role via OTHER group" residual check. CLI `railbase scim token create/list/revoke/rotate`. 7 unit + 6 e2e. Sort + etag explicitly `false` (IdPs ignore). | C | ✅ (v1.7.45 + v1.7.47) |
...
### 3.11 ✅ Admin UI screens (v1.5-v1.7) — функционально полно для v1
...
| **System admin tables** (read-only `_admins`/`_sessions`/`_admin_sessions`) | ✅ (v1.7.46a — `internal/api/adminapi/system_tables.go` + `admin/src/screens/system_{admins,sessions,admin_sessions}.tsx`; revoke-session affordance; admin lifecycle stays on CLI) | M | 3.2 |
| **Backend metrics dashboard** (goroutines / heap / pool / jobs / hooks latency) | ✅ (v1.7.46b — `internal/metrics/` + `internal/api/adminapi/metrics.go` + `admin/src/screens/dashboard.tsx` polling 5s) | M | docs/14 |
| **Admin recovery flows** (forgot password + reset password) | ✅ (v1.7.46d — `internal/api/adminapi/forgot_password.go` + `_record_tokens` purpose `admin_password_reset` + mailer builtin `admin_password_reset.md`) | M | 3.1 |
| **Setup wizard (4-step DB / Mailer / Auth / Admin)** | ✅ (v1.7.39 DB step + v1.7.43 Mailer step + v1.7.45 Auth step + Admin step; SMTP presets Google/MailHog/Custom; SAML/LDAP/SCIM/OAuth toggles) | L | 3.2 / 3.1 / 2.4 |
| **UI kit color tokens ESLint enforcement** | ✅ (v1.7.46c — `admin/eslint.config.js` + custom `admin/eslint-rules/no-raw-colors.js` блокирует raw `bg-red-*`/`text-amber-*` вне `lib/ui/`) | S | 2.8.1 |
...
### 3.15 🔄 Не реализованный функционал (post v1.7.47 audit)

Аудит проведён 2026-05-13 после v1.7.47. Группы упорядочены по приоритету для v1.x bonus burndown.

### A. Известные deferred внутри отгруженных подсистем

| Пункт | Где | Статус | Решение |
|---|---|---|---|
| **SCIM `soft_delete` setting** | ~~`internal/api/scim/users.go`~~ | ✅ (v1.7.48a) | `auth.scim.soft_delete` toggle implemented; UPDATE `deleted=now()` для SoftDelete-enabled collections; reconcileGroupGrants drops всё равно прокатываются; 4 e2e |
| **SCIM sort + etag** | ~~`internal/api/scim/discovery.go`~~ | ✅ (v1.7.48c) | RFC 7644 §3.4.2.3+§3.7 full implementation; `?sortBy=`+`?sortOrder=` w/ whitelist; weak ETag `W/"<epoch_ms>"`; `If-None-Match`→304, `If-Match` mismatch→412, `If-Match: *` wildcard; `.ServiceProviderConfig.sort.supported`+`etag.supported` теперь `true`; 8 e2e |
| **SAML xmlenc (encrypted assertions)** | `internal/auth/saml/saml.go:40-79` — комментарий "no encrypted assertions" | 📋 v1.x | Post-binary-budget (бинарник +~500 KB на xmlenc); закроется когда headroom вырастет |
| **SAML XSW-2/XSW-7 wrapping attack hardening** | `internal/auth/saml/saml.go:73-79` — комментарий "beyond goxmldsig native" | 📋 v1.x | goxmldsig покрывает XSW-1; полная защита от Signature Wrapping = отдельный slice |
| **LDAP connection pooling** | `internal/auth/ldap/ldap.go` — deliberately new connection per request | 📋 v1.x | Сейчас не нужно для enterprise low-traffic regime; first-connection latency <50ms на STARTTLS |
| **goja per-VM memory limit** | `internal/hooks/loader.go` — stack-cap есть (v1.7.31d), heap-cap нет | 📋 upstream-blocked | goja не экспортирует heap-limit API; ждать upstream PR или V8/QuickJS swap |
| **JS-side `mockApp().fireHook()` harness** | §3.12.8 закрыт Go-side через `MockHookRuntime`; pure-JS testing | 📋 v1.x | Closes when JS hook authors попросят inline test API; Go-side `testapp.NewMockHookRuntime(t)` сейчас достаточен |
| **Webhooks SDK strict-mode SSE expand** | `internal/realtime/pb_compat.go` — `?expand=` row resolution | 📋 v1.x | 1-2 day slice — не блокирует PB SDK drop-in compat (8/9 закрыто) |
| **Audit log export** | ~~`admin/src/screens/audit.tsx`~~ | ✅ (v1.7.48b) | `GET /api/_admin/audit/export.xlsx` streaming через `excelize.StreamWriter`; 100k hard cap w/ `X-Truncated` header; admin UI кнопка "Export XLSX"; audit-of-audit `audit.exported` event; 4 e2e; bundle +0.95 KB raw |
| **Mailer template edit-via-UI** | `admin/src/screens/mailer.tsx` — preview есть, save нет | 📋 by-design | Operator workflow стабильно "edit `.md` + hot-reload"; template source в git, не DB |

### B. Заблокированные функционалом v1.x+ (dependency-blocked)

| Пункт | Зависит от | Эффорт when unblocked |
|---|---|---|
| Documents browser (admin UI) | §3.6 Documents track (logical entity + versions + retention) — out of v1 scope | L |
| Hierarchical tree/DAG visualizers (admin UI) | §3.8 Hierarchies tail (closure_table + nested_set + DAG) — deferred v1.6.x | L |
| Realtime collaboration indicators | v1.1+ (presence layer на WebSocket transport) | M |
| Devices, invites, impersonation | v1.1.x — `_devices` table + admin impersonation cookie | L |
| `document_retention` + `thumbnail_generate` builtins | §3.6 Documents track | S каждая |
| `text_extract` job builtin (OCR/PDF parsing) | v1.2 plugin `railbase-doc-ocr` | M |
| Documents storage S3/GCS drivers | §3.6.12 — admin UI + plugin host | M |

### C. Explicit v1.1+ track (по docs/16 roadmap)

| Подсистема | Документ | Эффорт |
|---|---|---|
| Plugin host (custom gRPC, manifest, distribution) | docs/15 | XL |
| `railbase-cluster` (NATS distributed broker) | docs/15 | L |
| `railbase-orgs` (organizations, invites, seats) | docs/15 | XL |
| `railbase-billing` (Stripe primary) | docs/15 | XL |
| S3 storage adapter | docs/07 | M |
| OpenTelemetry (OTLP/HTTP) | docs/14 | M |
| Prometheus `/metrics` (HTTP endpoint export, не in-process counters v1.7.46) | docs/14 | S |
| Encryption at rest (AES-256-GCM field-level + KMS) | docs/14 | L |
| Self-update mechanism (`railbase update` / `rollback`) | docs/14 | L |
| Postgres production hardening guide | docs/03 | L |
| Schema-per-tenant escape hatch | docs/03 | M |

### D. Действительно не начато / out-of-v1

- **Mobile SDKs** (Swift, Kotlin, Dart) — docs/11 — v1.2
- **`railbase-mcp`** (LLM agents) — docs/15 — v1.2
- **`railbase-saml-encrypt`** (xmlenc плагин если xmlenc не закроем в ядре) — v1.2 candidate
- **BPMN authoring в admin UI** — v2
- **Federated/multi-region replication** — v2
- **`--template ai`** (pgvector), **`--template enterprise`** (полный SAML+SCIM+sealing preset) — v1.2

### Сводка

- Полностью закрытые сейчас (post v1.7.47): **8/9 §3.x групп**, Enterprise SSO в ядре, Admin UI 22/25 screens, тест-coverage 49 пакетов под `-race`
- 📋 в **отгруженных** подсистемах: **7 deferred** (А-блок, was 10 — v1.7.48 закрыл 3: SCIM soft_delete + audit XLSX export + SCIM sort/etag) — все мелкие slice'ы, не блокируют v1 SHIP
- 📋 **dependency-blocked**: **7** (B-блок) — закроются как §3.6 Documents / §3.8 Hierarchies tail / v1.1+ plugin host шипяться
- 📋 **explicit v1.1+**: **11** (C-блок) — отдельный roadmap track, ~6 недель after v1
- 📋 **out-of-v1**: **6** (D-блок) — v1.2+ или v2

**Honest call**: v1 SHIP полностью разблокирован. Оставшийся `📋`-marker surface — это (i) сознательные deferred внутри shipped кода (А-блок, ~10 slice-days если набрать), (ii) features, ждущие зависимостей (B), (iii) v1.1+/v1.2/v2 roadmap (C/D). Никакой из 📋 пунктов НЕ блокирует выпуск v1.
