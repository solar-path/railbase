...
### ~~railbase-authority~~ → moved to core (v2.0)

**Reclassified 2026-05-16.** Authority (DoA) изначально планировался
как plugin по образцу rail. По результатам:

- Два независимых embedder feedback'а (blogger §K editorial approval,
  sentinel ERP financial JE materiality) явно запросили primitive.
- Bypass discipline (no-skip из hooks/jobs/import) невозможно
  обеспечить из plugin'а — gate должен сидеть в фиксированной точке
  core stack'а.
- Audit chain integration требует доступ к core sealed-chain
  механизму, который не открыт plugin'ам.
- Аналогия с per-tenant RBAC, который тоже сначала был plugin, а
  потом ушёл в core по тем же причинам.

**Новое место**: [26-authority.md](26-authority.md) — design proposal
для v2.0 (rev2 после ysollo comparison analysis).

Architecture: **hybrid schema-as-code + matrix-as-data**.
Schema объявляет gate points через
`.Authority({Matrix: "key", On: ..., ProtectedFields: ...})`.
Actual approval rules — runtime data в `_doa_matrices` /
`_doa_matrix_levels` / `_doa_matrix_approvers`, editable через admin
UI с version history. Runtime: `_doa_workflows` /
`_doa_workflow_decisions` / `_doa_delegations`. Sealed audit chain
через `target='authority'` в `_audit_seals`.

Approver types: `role / user / position / department_head`. Per-level
modes: `any / all / threshold`. Escalation timeouts per level.
Delegation first-class (v2.0, не v2.1).

**Spawning tasks**: каждый level workflow'а порождает task per
qualified approver через core tasks primitive
([27-tasks.md](27-tasks.md), тоже v2.0).

**Plugin matrix namespace ownership** (см. [26 §Matrix key namespacing](26-authority.md)):

Plugin регистрирует свой namespace `<plugin-id>.*` на install. Все
matrices, declared в plugin'овых `.Authority({Matrix: "..."})`,
должны иметь этот prefix. Например `railbase-billing` plugin
владеет `billing.*` — может registerать `billing.refund.large`,
`billing.subscription.cancel`, etc.

Три-tier rule:
- `system.*` — reserved для core Railbase (никто другой)
- `<plugin-id>.*` — strict per plugin, registry validates на install
- bare embedder names (default) OR `app.*` opt-in — для application schema

Conflicts:
- Embedder schema register → registry проверяет prefix против
  installed plugins; collision → error с rename suggestion.
- Plugin install → registry проверяет вне-namespace embedder keys;
  conflicting bare keys hard-fail с migration suggestion.

Plugin uninstall с running workflows:
- Default: refuse uninstall если active workflows ссылаются на
  plugin matrices.
- Opt-in `--force --archive-workflows`: cancel all running с
  audit event + admin responsibility.

См. [26-authority.md §Matrix key namespacing](26-authority.md) для
полного спека.
...
| `saas` | railbase-orgs, railbase-billing, railbase-fx |
| `saas-erp` | + 38-ролевой каталог seed, railbase-fx, multi-currency examples, `.Authority()` примеры (v2.0) |
| `mobile` | railbase-push |
| `ai` | railbase-mcp, --with-vec flag |
| `enterprise` | + railbase-saml, railbase-scim, audit sealing, `.Authority()` примеры (v2.0) |
| `fintech` | railbase-fx, railbase-billing, audit sealing, `.Authority()` с materiality threshold (v2.0), --with-crypto-currencies |

> Note: `railbase-authority` исчез из plugin списка с 2026-05-16
> — DoA reclassified в core ([docs/26-authority.md](26-authority.md), v2.0).
> Templates выше теперь полагаются на core `.Authority()` DSL,
> а не на отдельный plugin.
