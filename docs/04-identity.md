...
### Auth response includes custom profile fields

`POST /api/collections/<n>/auth-with-password` (а также `auth-signup`
и `auth-refresh`) возвращает в `record` **все** non-secret поля
коллекции, не только системные. То есть для
`AuthCollection("authors").Field("display_name", ...).Field("bio",
...)` ответ содержит:
...
```json
{
  ...
  "record": {
    ...
    "verified": true,
    "last_login_at": "2026-05-16 01:13:58.000Z",
    "display_name": "Alice Anderson",
    "bio": "FT correspondent",
    "created": "...",
    "updated": "..."
  }
}
...
`email/password_hash/token_key/verified/last_login_at` всегда
системные; их формат не зависит от user-declared полей. Custom-поля
с теми же именами не могут перезаписать системные (defensive merge в
`writeAuthResponse`). M2M-relations пропускаются (живут в junction
tables, не в основной строке). Implementation —
`internal/api/auth/auth.go::fetchAuthCustomFields`.

### Public profile read endpoint (опт-ин)

`AuthCollection("authors").PublicProfile()` включает анонимный
read-only endpoint:

```
GET /api/collections/{name}/profiles            # list (LIMIT 1000)
GET /api/collections/{name}/profiles/{id}        # single
```

Возвращаются **только** `id` + user-declared поля; secrets
(`email`, `password_hash`, `token_key`, `verified`, `last_login_at`)
**никогда не появляются**. Use case — рендер bylines, авторов,
contributors на анонимных страницах CMS / магазина / блога.

На collections без `.PublicProfile()` endpoint возвращает 404 (та же
форма, что для несуществующих коллекций — probers не могут
энумерировать какие коллекции опт-ин). Implementation —
`internal/api/rest/profiles_handler.go`.

> **Rule grammar**: «collectionName», не «collection». В правилах
> вида `@request.auth.collectionName = 'authors'`. Если написать
> singular — компилятор фильтра 400-ит ответом, в котором ясно
> названа позиция, неверный токен и список допустимых magic-vars
> (FEEDBACK #B3).
...
### Композиция с DoA — RBAC сначала, потом authority gate

RBAC отвечает на «**может** ли этот пользователь делать это
действие сейчас». [26-authority.md](26-authority.md) (Delegation
of Authority, v2.0+) отвечает на «**можно** ли этому изменению
вообще произойти» (требует ли санкции N акторов).

Они композируются — DoA gate стоит **после** RBAC в request
pipeline:

```
HTTP → Auth → Tenant → LoadActions →
   RBAC.Require(action)       ← может 403 "permission denied"
   Authority.Gate(transition) ← может 409 "authority required"
   handler logic
```

Семантически:
- RBAC отказывает → запрос отбит как «у тебя нет прав». DoA не
  вызывается.
- RBAC пропускает → DoA смотрит, gated ли конкретный transition
  (через `.Authority()` на коллекции). Если да и нет approved
  request — 409 с инструкцией как создать authority request.

Пример с blogger newsroom (rev2 hybrid model):

```go
// Schema: gate point declaration. NO inline rules.
Articles.
    UpdateRule(`@request.auth.id = author OR
                @request.auth.role IN ('editor','chief')`).
    Authority(railschema.AuthorityConfig{
        On:              railschema.AuthorityOn{Op:"update", Field:"status", To: []string{"published"}},
        Matrix:          "article.publish",       // ← ссылка на matrix-by-key
...
```
// Matrix: actual rules, edited через admin UI или CLI.
Matrix "article.publish" v1, approved:
  Level 1: "Editorial review" — mode=any, approvers=[role:editor]
    escalation: 24h → level 2
  Level 2: "Chief sign-off" — mode=any, approvers=[role:chief]
    escalation: 48h → expire
```

Reading: «UpdateRule даёт право автору ИЛИ editor/chief писать в
эту строку (RBAC). Любое изменение status в published требует
санкции — найти applicable matrix, создать workflow, пройти все
levels (DoA)».

Author с RBAC-разрешением на Update **всё равно** получит 409
при попытке сразу сэтнуть `status='published'` — это разные оси
compliance, и они работают вместе.

Helpers:
- `authority.QualifiedApprover(workflow_id)` — RBAC predicate
  «текущий пользователь подходит как approver для workflow_id»
  (учитывая active delegations + approver type resolution).
  Используется в default rule для `/api/authority/workflows/
  {id}/approve`.

См. [26-authority.md](26-authority.md) для полного спека DoA
(rev2 hybrid model).
